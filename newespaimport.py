...
# 3. Converts SR, BT, and Fmask data from UTM to Irish Transverse Mercator (ITM) projection.
...
## main
parser = argparse.ArgumentParser('This script imports LEDAPS-processed scenes into the local library. It stacks images and converts them to Irish Transverse Mercator projection, and adds ENVI metadata.')
...
parser.add_argument('-f','--fmaskdir', type = str, default = ieo.fmaskdir, help = 'Directory containing FMask cloud masks in Irish Transverse Mercator projection.')
...
for dir in [args.outdir, os.path.join(args.outdir, 'L1G')]:
    rlist = glob.glob(os.path.join(args.outdir, '*_ref_ITM.dat'))
    for f in rlist:
